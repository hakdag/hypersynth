...
#[tokio::test]
async fn task_priority_accepts_allowed_values_and_rejects_invalid() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    sqlx::query(
        ...
        VALUES ($1, 'SF30 Co', $2, 'TR', 'UTC', 'active')
        "#,
    )
    .bind(company_id)
    .bind(format!("sf30-priority-{}@example.test", Uuid::new_v4()))
    .execute(&mut *tx)
    .await?;
    ...
    for priority in ["Standard", "Elevated", "Critical"] {
        sqlx::query(
            r#"
            INSERT INTO tasks (id, feature_id, title, description, status, created_by, priority)
            VALUES ($1, $2, $3, NULL, 'Pending', 'User', $4)
            "#,
        )
        .bind(Uuid::new_v4())
        .bind(feature_id)
    ...
    let invalid_result = sqlx::query(
        r#"
        INSERT INTO tasks (id, feature_id, title, description, status, created_by, priority)
        VALUES ($1, $2, 'Invalid task', NULL, 'Pending', 'User', 'Urgent')
        "#,
    )
    .bind(Uuid::new_v4())
    .bind(feature_id)
...
#[tokio::test]
async fn task_priority_change_event_can_be_persisted() -> Result<(), Box<dyn std::error::Error>> {
    let Some(pool) = connect_pool().await else {
        return Ok(());
    };
    sqlx::migrate!("./migrations").run(&pool).await?;
    ...
    sqlx::query(
        ...
        ) VALUES ($1, 'task_priority_changed', $2, $3, $4, NULL, 'test')
        "#,
    )
    ...
    let count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM audit_events WHERE event_type = 'task_priority_changed'",
    )
    .fetch_one(&pool)
    .await?;
    assert!(count >= 1);
